...
var datepanel = require("panel").Panel({
  ...
  onMessage: function(info) {
    ...
    widgetClick("https://api-dev.bugzilla.mozilla.org/latest/bug?" + info +
        "&include_fields=id,summary,assigned_to,creation_time,last_change_time,priority,status");
  }
});
...
function listAllBugsAndNames(jsonbugs, type) {
    ...
    for(i in jsonbugs) {
      if(jsonbugs[i].status != "RESOLVED" && jsonbugs[i].status != "VERIFIED" && jsonbugs[i].status != "CLOSED") { 
        count = count + 1;
        goodbugs.push(jsonbugs[i]);
      }
    }
    var bugtab;
